        ...
        ->routes(
            ...
            Route::get('[/page/{page:\d+}]')
                ...
                ->middleware(
                    ...
                    $httpCache->withLastModified(function (ServerRequestInterface $request, $params) use ($readPostQueryService) {
            ...
            Route::get('/post/{slug}')
                ->name('blog/post')
                ->middleware(
                    fn (HttpCache $httpCache, ReadPostQueryServiceInterface $readPostQueryService, CurrentRoute $currentRoute) =>
                    $httpCache->withEtagSeed(function (ServerRequestInterface $request, $params) use ($readPostQueryService, $currentRoute) {
                        $post = $readPostQueryService->getPostBySlug($currentRoute->getArgument('slug'));
                        return $post === null ? 'no-post' : $post->getSlug() . '-' . $post->getUpdatedAt()->getTimestamp();
                    })
                )
            ...
            Group::create('/author')
                ->routes(
                    ...
                    Route::get( '/{author}/posts[/page/{page}]')
                        ...
                        ->middleware(fn (AccessPermissionChecker $checker) => $checker->withPermission('authorPostsList'))
                    ...
                    Route::post('/post/delete/{slug}')
                        ...
                        ->middleware(fn (AccessPermissionChecker $checker) => $checker->withPermission('authorDeletePost'))
                        ->middleware(Authentication::class)
